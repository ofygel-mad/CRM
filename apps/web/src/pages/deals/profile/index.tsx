...
const ACTIVITY_CONFIG: Record<string, { icon: typeof Clock; color: string; label: string }> = {
...
function InlineAmount({
  ...
  return (
    ...
    >
      <span style={{ fontSize: 26, fontWeight: 800, color: 'var(--color-amber)', fontFamily: 'var(--font-display)' }}>
        {value ? (
          `${fmt(value)} ${symbol}`
        ) : (
          <span style={{ color: 'var(--color-text-muted)', fontWeight: 400, fontSize: 15 }}>Сумма не указана</span>
        )}
      </span>
      <Edit3 size={13} style={{ color: 'var(--color-text-muted)', opacity: 0.4 }} />
    </motion.div>
  );
}
    ...
    <div>
      <div style={{ display: 'flex', gap: 3 }}>
        {sorted.map((s, i) => {
          ...
          const color =
            s.type === 'won' ? 'var(--color-success)' : s.type === 'lost' ? 'var(--color-danger)' : 'var(--color-amber)';
          return (
            <motion.button
              key={s.id}
              ...
              style={{
                ...
                background: isActive ? color : isPast ? `${color}55` : 'var(--color-border)',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
        <span style={{ fontSize: 12, color: 'var(--color-amber)', fontWeight: 600 }}>
      ...
      <AnimatePresence>
        {focused && (
          ...
          >
            ...
            <Button size="sm" loading={add.isPending} onClick={() => body.trim() && add.mutate(body)}>
              Сохранить
            </Button>
          </motion.div>
        )}
...
function ActivityTimeline({ activities }: { activities: Activity[] }) {
  if (!activities.length) {
    return <EmptyState icon={<Clock size={20} />} title="Нет активностей" subtitle="История изменений появится здесь" />;
  }
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      {activities.map((act, idx) => {
        const cfg = ACTIVITY_CONFIG[act.type] ?? { icon: Clock, color: '#6B7280', label: act.type };
        const Icon = cfg.icon;
        ...
        return (
          <motion.div
            ...
            style={{ display: 'flex', gap: 10, padding: '10px 0', position: 'relative' }}
          >
            {idx < activities.length - 1 && (
              <div
                style={{ position: 'absolute', left: 13, top: 36, bottom: 0, width: 1, background: 'var(--color-border)' }}
              />
            )}
            <div
              style={{
            ...
            <div style={{ paddingTop: 3 }}>
              <div style={{ fontSize: 12, color: 'var(--color-text-primary)', lineHeight: 1.4 }}>{text}</div>
              <div style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 2 }}>
                {act.actor && <b style={{ color: 'var(--color-text-secondary)' }}>{act.actor.full_name}</b>}
                {act.actor && ' · '}
...
function CustomerCard({ customer, onClick }: { customer: DealDetail['customer']; onClick: () => void }) {
  if (!customer) {
    return (
      <div style={{ border: '1px dashed var(--color-border)', borderRadius: 'var(--radius-lg)', padding: 16, textAlign: 'center' }}>
        <User size={18} style={{ color: 'var(--color-text-muted)', marginBottom: 6 }} />
        <p style={{ fontSize: 12, color: 'var(--color-text-muted)', margin: 0 }}>Клиент не привязан</p>
      </div>
    );
  }
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        ...
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-text-primary)' }}>{customer.full_name}</div>
...
const TAB_LABELS: Record<Tab, string> = { activity: 'Активность', notes: 'Заметки', tasks: 'Задачи' };
    ...
    <div style={{ padding: '24px 28px', maxWidth: 1120 }}>
        ...
        <div>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 18, gap: 12 }}>
              <div>
                <h1 style={{ fontSize: 20, fontWeight: 700, fontFamily: 'var(--font-display)', margin: '0 0 6px' }}>{deal.title}</h1>
                <InlineAmount value={deal.amount} currency={deal.currency} onSave={(v) => updateAmount.mutate(v)} />
              </div>
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                <Badge
                  color={deal.status === 'won' ? 'var(--color-success)' : deal.status === 'lost' ? 'var(--color-danger)' : 'var(--color-warning)'}
                  bg={deal.status === 'won' ? 'var(--color-success-light)' : deal.status === 'lost' ? 'var(--color-danger-light)' : 'var(--color-warning-light)'}
                >
          ...
          <AnimatePresence mode="wait">
            ...
            >
                    ...
                    .map((a) => (
                      <div key={a.id} style={{ padding: '10px 0', borderTop: '1px solid var(--color-border)' }}>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 4 }}>
                          {a.actor?.full_name} · {formatDistanceToNow(new Date(a.created_at), { addSuffix: true, locale: ru })}
                        </div>
                      </div>
                    ))}
                </>
              ...
              {tab === 'tasks' && (
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                    <span style={{ fontSize: 13, fontWeight: 600 }}>Задачи</span>
                    ...
                    (tasks?.results ?? []).map((task) => (
                      ...
                      >
                        <input
                          ...
                          style={{ width: 16, height: 16, cursor: 'pointer', accentColor: 'var(--color-amber)' }}
                        />
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 13, textDecoration: task.is_done ? 'line-through' : 'none' }}>{task.title}</div>
                          {task.due_date && (
              ...
              {tab === 'activity' && <ActivityTimeline activities={activities?.results ?? []} />}
            </motion.div>
          </AnimatePresence>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <CustomerCard customer={deal.customer} onClick={() => deal.customer && navigate(`/customers/${deal.customer.id}`)} />
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {deal.owner && <MetaRow icon={<User size={13} />} label="Ответственный" value={deal.owner.full_name} />}
              <MetaRow
                icon={<Calendar size={13} />}
                label="Создана"
                value={format(new Date(deal.created_at), 'd MMM yyyy', { locale: ru })}
              />
              {deal.expected_close_date && (
                <MetaRow
                  icon={<Target size={13} />}
                  label="Дата закрытия"
                  value={format(new Date(deal.expected_close_date), 'd MMM yyyy', { locale: ru })}
                  extra={
                    daysToClose !== null && (
                      <span
                        style={{
              ...
              <MetaRow icon={<AlertCircle size={13} />} label="Воронка" value={deal.pipeline.name} />
            </div>
          </div>
        </div>
      </div>
      ...
      <Drawer
        ...
        footer={
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            ...
            <Button loading={isSubmitting} onClick={handleSubmit((d) => updateDeal.mutate(d))}>
              Сохранить
            </Button>
          </div>
        }
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <Field label="Сумма">
            <input {...register('amount')} type="number" className="crm-input" />
          </Field>
          <Field label="Дата закрытия">
            <input {...register('expected_close_date')} type="date" className="crm-input" />
          </Field>
        </div>
      </Drawer>
    </div>
